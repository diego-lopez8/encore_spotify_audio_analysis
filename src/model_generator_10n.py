...
This file specifically deals with running models sampled with 10% additional noise injected
...
noisy_sample_df_10 = pd.DataFrame()
for key in geneva_scale.keys(): 
    print(key)
    length = len(full_scaled_df[full_scaled_df["emotion"] == key])
    # sample from correct labeled distribution
    noisy_sample_df_10 = pd.concat([noisy_sample_df_10, full_scaled_df[full_scaled_df["emotion"] == key].sample(frac=0.9, replace=False)])
    # sample from noisy distribution with wrong labels
    noisy_sample_df_10 = pd.concat([noisy_sample_df_10, full_scaled_df[full_scaled_df['emotion'] != key].sample(n=int(0.1*length), replace=False).assign(emotion=key)])

full_scaled_df.dropna(inplace=True)
noisy_sample_df_10.dropna(inplace=True)
print("length of dataset with every label having +10% noise" ,len(noisy_sample_df_10))
print("length of weakly supervised dataset", len(full_scaled_df))
print("resampling off")
X_train_10n = noisy_sample_df_10.iloc[:,:-1]
y_train_10n = noisy_sample_df_10.iloc[:,-1]

# Random Forest
# same hyperparameters as RF no noise
clf_rf_10n = RandomForestClassifier(criterion="gini", max_depth=50, min_samples_split=20, n_estimators=250, n_jobs=-1)
print("[INFO] Starting Random Forest...")
clf_rf_10n.fit(X_train_10n, y_train_10n)
with open("clf_rf_10n.pkl", "wb") as f:
    pickle.dump(clf_rf_10n, f)
print("Completed Random Forest...")

# Logistic Regression
clf_lr_10n = LogisticRegressionCV(Cs=10, fit_intercept=False, cv=cv, penalty="elasticnet", solver="saga", max_iter=1000, l1_ratios=[0,0.25, 0.5, 0.75, 1], verbose=1, n_jobs=-1)
print("[INFO] Starting Logistic Regression...")
clf_lr_10n.fit(X_train_10n, y_train_10n)
with open("clf_lr_10n.pkl", "wb") as f:
    pickle.dump(clf_lr_10n, f)
print("Completed Logistic Regression...")

# Ada Boost
clf_abc_10n = AdaBoostClassifier(random_state=0, learning_rate=0.5, n_estimators=500)
print("[INFO] Starting Ada Boost...")
clf_abc_10n.fit(X_train_10n, y_train_10n)
with open("clf_abc_10n.pkl", "wb") as f:
    pickle.dump(clf_abc_10n, f)
print("Completed Ada Boost...")

# Decision Tree
clf_dtc_10n = DecisionTreeClassifier(criterion="gini", max_depth=None, max_features=None)
print("[INFO] Starting Decision Tree...")
clf_dtc_10n.fit(X_train_10n, y_train_10n)
with open("clf_dtc_10n.pkl", "wb") as f:
    pickle.dump(clf_dtc_10n, f)
print("Completed Decision Tree...")

# KN Classifier
clf_knc_10n = KNeighborsClassifier(algorithm="ball_tree", leaf_size=40, n_neighbors=20, p=1)
print("[INFO] Starting KN Classifier...")
clf_knc_10n.fit(X_train_10n, y_train_10n)
with open("clf_knc_10n.pkl", "wb") as f:
    pickle.dump(clf_knc_10n, f)
print("Completed KN Classifier...")

# Gradient Boost
clf_gbc_10n = GradientBoostingClassifier(learning_rate=0.1, loss="deviance", max_depth=10, max_features="sqrt", n_estimators=400)
print("[INFO] Starting Gradient Boost...")
clf_gbc_10n.fit(X_train_10n, y_train_10n)
with open("clf_gbc_10n.pkl", "wb") as f:
    pickle.dump(clf_gbc_10n, f)
print("Completed Gradient Boost...")

# MLP
clf_mlp_10n = MLPClassifier(activation="relu", alpha=0.0001, batch_size=128, hidden_layer_sizes=(10,10,10), 
                            learning_rate="adaptive", learning_rate_init=0.001, solver="adam")
print("[INFO] Starting MLP Classifier...")
clf_mlp_10n.fit(X_train_10n, y_train_10n)
with open("clf_mlp_10n.pkl", "wb") as f:
    pickle.dump(clf_mlp_10n, f)
